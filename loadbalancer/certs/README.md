...
`server.key`). Es ist ein reines Dev-/Demo-Zertifikat fuer dieses Schulprojekt
...
werden (bei `curl` das Flag `-k` verwenden). Fuer die internen
Self-Signed-Verbindungen ist die Zertifikatspruefung in Docker Compose
deaktiviert (`PEER_TLS_VERIFY=false`, `proxy_ssl_verify off`); die Verbindung
ist trotzdem TLS-verschluesselt.
